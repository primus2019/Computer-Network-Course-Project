...
countrynames = ('Argentina', 'Australia', 'Belgium', 'Brazil', 'Canada', 'China', 'Denmark', \
        ...
        'Sweden', 'Switzerland')
cnames = tk.StringVar(value=countrynames)
...
def showPopulation(*args):
    idxs = lbox.curselection() ################################################ important
    if len(idxs)==1:
        idx = int(idxs[0])
...
status = ttk.Label(c, textvariable=statusmsg, anchor=tk.W)

# Grid all the widgets
lbox.grid(column=0, row=0, rowspan=6, sticky=(tk.N,tk.S,tk.E,tk.W))
lbl.grid(column=1, row=0, padx=10, pady=5)
g1.grid(column=1, row=1, sticky=tk.W, padx=20)
g2.grid(column=1, row=2, sticky=tk.W, padx=20)
g3.grid(column=1, row=3, sticky=tk.W, padx=20)
send.grid(column=2, row=4, sticky=tk.E)
sentlbl.grid(column=1, row=5, columnspan=2, sticky=tk.N, pady=5, padx=5)
status.grid(column=0, row=6, columnspan=2, sticky=(tk.W,tk.E))
